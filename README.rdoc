...
and unpacking on +read_packed+. For example:
  ...
  class MyHeader
    include Packable
    attr_accessor :signature, :nb_blocks

    def write_packed(packedio, options)
      packedio << [signature, {:bytes=>4}] << [nb_block, :short]
    end
    
    def self.read_packed(packedio, options)
      h = MyHeader.new
      h.signature, h.nb_blocks = packedio >> [String, {:bytes => 4}] >> :short
      h
    end
  end
...
It is not necessary to return +self+. The previous example can thus be shortened:
  ...
  class MyHeader
    ...
    def read_packed(packedio, options)
      signature, nb_blocks = packedio >> [String, {:bytes => 4}] >> :short
    end
  end
...
packable is licensed under the terms of the (modified) GPL License, see the included LICENSE file.
